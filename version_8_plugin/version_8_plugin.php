 ...
 * @link
 */
if ( ! function_exists( 'site_include_post_by_id' ) ) :
 ...
 * @link
 */
if ( ! function_exists( 'site_include_post_by_cat' ) ) :
function site_include_post_by_cat( $attr )
{
    /*
    *************************************
    ...
    if ( !is_null( $cat ) && ( is_numeric( $cat ) || preg_match( '/^[0-9,]+$/', $cat ) ) && !is_feed() )
    {
        ...
        if(is_array($post_array) && count($post_array) > 0)
        {
            foreach($post_array as $item)
            {
                ...
                $output .= site_include_post_by_id($args);
            }
...
add_shortcode( 'site_include_post_by_cat', 'site_include_post_by_cat' );
endif;


/**
 * include post excerpt
 * deprecated before it was ever used, try site_include_post_by_id
 *
 * @link
 */
if ( ! function_exists( 'site_include_post_excerpt' ) ) :
...
/**
 * include post by category
 * OLD version, being replaced by one that uses the site_include_post_by_id function
 *
 * @link
 */
...
add_shortcode( 'site_include_post_by_category', 'site_include_post_by_category' );
endif;
